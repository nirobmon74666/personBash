...
# chrome_options.add_argument("--user-data-dir=/home/nirobmon74666/.config/google-chrome")
...
def logIn(user):
    ...
    print("\nPhone No. : 0{}, Password : {} and ID: {}".format(phone[user], password[user],user))
...
def mineDigging():
    ...
    try:
        if  driver.find_element(By.XPATH,"/html/body/div[16]/table/tbody/tr/td[2]/div/a[2]").text == 'Start Job' or driver.find_element(By.XPATH,"/html/body/div[16]/table/tbody/tr/td[2]/div/a[1]").text == "Continue Job":
...
def memoryGame():
    driver.get('https://gridplays.com/qute/job')
    try:
        if  driver.find_element(By.XPATH,"/html/body/div[13]/table/tbody/tr/td[2]/div/a[2]").text == 'Start Job' or driver.find_element(By.XPATH,"/html/body/div[13]/table/tbody/tr/td[2]/div/a[1]").text == "Continue Job":
...
def fruitCounting():
    driver.get('https://gridplays.com/qute/job')
    driver.get("https://gridplays.com/qute/counting/index/act/start/src/job?{}".format(getToken()))
    driver.get("https://gridplays.com/qute/counting/index/src/job")
    solFruit=[("10","8","9"), ('10',"8",'9'), ('8','9','10'), ('9','10','8'), ('8','9','10'), ('10','8','9'), ('10','8','9'), ('8','9','10'), ('9','10','8'), ('9','9','10'), ('11','10','9')]
    while True:
        imgLinksE=driver.find_elements(By.TAG_NAME,"img")
        for imgLinkE in imgLinksE:
            imgLink=imgLinkE.get_attribute("src")
            if imgLink[0:52]=="https://gridplays.com/qute/images/minigame/counting/":
                imgNum=int(os.path.splitext(os.path.basename(urlparse(imgLink).path))[0].split("_")[1])

        search_text=["Sphere grape","Diamond dew","Vegeterian beef"]
        get_source = driver.page_source
        for i in range(0,3):
            if search_text[i] in get_source:
                driver.find_element(By.LINK_TEXT,'{} pieces'.format(solFruit[imgNum-1][i])).click()

        get_source = driver.page_source
        # it=driver.find_element(By.XPATH,"/html/body/div[8]/a").text
        if "Get Job's Reward"in get_source:
            driver.find_element(By.LINK_TEXT,"Get Job's Reward").click()
            driver.find_element(By.LINK_TEXT,"Check another job").click()
            print("▣", end="")
            driver.get("https://gridplays.com/qute/counting/index/act/start/src/job?{}".format(getToken()))
...
condtn=input('Do you want to Start the Job first? \nPress "y": ')
user = 1
while user < 47:
    if condtn=="y":
        logIn(user)
